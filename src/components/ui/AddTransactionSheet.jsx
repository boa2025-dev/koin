        ...
        <div className="md:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3">
              ...
              <button
                onClick={handleClose}
                className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center text-brand-muted hover:text-brand-text transition-colors"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="px-5 pb-8 space-y-4">
              ...
              <div className="flex p-1 rounded-2xl" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
                {[
                  { v: 'expense', label: '↓ Gasto',   active: 'bg-brand-red/80 text-white' },
                  { v: 'income',  label: '↑ Ingreso',  active: 'bg-brand-green/80 text-brand-bg' },
                ].map(({ v, label, active }) => (
                  <button
                    key={v}
                    onClick={() => set('type', v)}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold font-dm transition-all cursor-pointer ${
                      form.type === v ? active : 'text-brand-muted'
                    }`}
                  >
                    {label}
                  </button>
              ...
              {/* Description */}
              <div>
                <p className="text-brand-muted text-xs mb-1.5">Descripción</p>
                <input
